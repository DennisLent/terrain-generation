...
impl SimplexNoise {
    ...
    pub fn generate_noise_map(nx: f32, ny: f32, size: f32, frequency: f32) -> f32 {
        let wavelength = size / frequency;
        let nx = nx  / size - 0.5;
        let ny = ny / size - 0.5;
        SimplexNoise::noise(nx / wavelength, ny /wavelength) as f32
    }
